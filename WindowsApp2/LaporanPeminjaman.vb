...
Public Class LaporanPeminjaman
    ...
    '    DataGridView1.Rows.Clear()
    '    Dim s As String = "select detail_pinjam.*, peminjaman.id_pegawai, peminjaman.tanggal_pinjam, peminjaman.tanggal_kembali, inventaris.nama, jenis.nama_jenis,ruang.nama_ruang, pegawai.nama_pegawai from detail_pinjam join peminjaman on detail_pinjam.id_peminjaman = peminjaman.id_peminjaman join inventaris on detail_pinjam.id_inventaris = inventaris.id_inventaris join jenis on inventaris.id_jenis = jenis.id_jenis join ruang on inventaris.id_ruang = ruang.id_ruang join pegawai on peminjaman.id_pegawai = pegawai.id_pegawai where peminjaman.tanggal_pinjam >= '" + DateTimePicker1.Value.ToString("yyyy-MM-dd") + "' and peminjaman .tanggal_pinjam <= '" + DateTimePicker2.Value.ToString("yyyy-MM-dd") + "'"
    '    Dim cmd As New OdbcCommand With {.CommandText = s, .Connection = koneksi}
    '    koneksi.Open()
    '    Dim dr As OdbcDataReader
    '    dr = cmd.ExecuteReader
    ...
    Private Sub loadkode()
        Dim s As String = "select kode_peminjaman from peminjaman"
        Dim cmd As New OdbcCommand With {.CommandText = s, .Connection = koneksi}
        koneksi.Open()
        Dim dr As OdbcDataReader
        dr = cmd.ExecuteReader
        While dr.Read
            ComboBox1.Items.Add(dr.Item("kode_peminjaman"))
        End While
        koneksi.Close()
    End Sub
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        koneksi.Open()
        Dim ds As New DataSet1
        Dim da As New OdbcDataAdapter("select detail_pinjam.kode_peminjaman, detail_pinjam.kode_inventaris,detail_pinjam.jumlah, peminjaman.tanggal_pinjam, peminjaman.tanggal_kembali, inventaris.nama, jenis.nama_jenis,ruang.nama_ruang, pegawai.nama_pegawai from detail_pinjam join peminjaman on detail_pinjam.id_peminjaman = peminjaman.id_peminjaman join inventaris on detail_pinjam.id_inventaris = inventaris.id_inventaris join jenis on inventaris.id_jenis = jenis.id_jenis join ruang on inventaris.id_ruang = ruang.id_ruang join pegawai on peminjaman.id_pegawai = pegawai.id_pegawai where peminjaman.tanggal_pinjam >= '" + DateTimePicker1.Value.ToString("yyyy-MM-dd") + "' and peminjaman.tanggal_pinjam <= '" + DateTimePicker2.Value.ToString("yyyy-MM-dd") + "'", koneksi)
        da.Fill(ds.Tables("DataTable1"))
        koneksi.Close()
    ...
    Private Sub LaporanPeminjaman_Closing(sender As Object, e As CancelEventArgs) Handles Me.Closing
        LaporanNav.Show()
        Me.Hide()
    End Sub
